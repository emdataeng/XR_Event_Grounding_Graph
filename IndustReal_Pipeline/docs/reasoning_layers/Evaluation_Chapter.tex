...
outputs. The evaluation therefore examines four aspects: artifact correctness,
constraint inference coverage, validation behavior under uncertain or conflicting
evidence, and graph-based traceability.
...
This situation occurs in the baseline artifact-correctness run for Step~9, \textit{Remove front wheel assy}. The step is preserved in the input sequence and has symbolic evidence, but the baseline rule configuration used in this check did not define removal-specific reasoning rules. The pipeline therefore records a rule-coverage warning instead of treating the absence of constraints as implicit success.
...
Across all evaluated clips, Layer 3 produced 1673 inferred constraints from 6568 predicate records: 643 \texttt{produces}, 776 \texttt{requires}, 67 \texttt{requiresTool}, 171 \texttt{requiresSafety}, and 16 incompatibility constraints. Remove-action semantics were covered for all 149 observed removal steps.
...
The evaluated graph contained 34 step nodes, 326 predicate nodes, 79 constraint
nodes, 8 rule nodes, 13 entity nodes, and 40 source nodes. The exported relations
included 33 \path{NEXT} edges, 27 \path{DEPENDS_ON} edges, 45
\path{REQUIRES} edges, 32 \path{PRODUCES} edges, 326 \path{HAS_PREDICATE}
edges, and 8 \path{INVALIDATED_BY} edges. The step status distribution was the
same as in Evaluation 3: 18 accepted steps, 10 uncertain steps, and 6 rejected
steps.
...
PASS: 79 constraints checked; 0 failures. \\
...
PASS: 3 uncertain-support dependencies checked; 0 failures. \\
...
\paragraph{Description:}
The evaluation uses the same \texttt{08\_assy\_0\_1} clip in the
\path{od_plus_psr_error_hints} mode as Evaluations 3 and 4. The baseline validation distribution contained 18 accepted steps, 10 uncertain steps, and 6 rejected steps.
...
steps was removed. In that scenario, seven dependent steps were affected. One
previously uncertain step became rejected, and six previously accepted steps
became rejected. For each affected dependent step, the dependency support was
...
The evaluation focuses on the reasoning contribution of this thesis. It checks that the implemented pipeline produces the expected artifacts, that Layer 3 enriches symbolic predicates with procedural constraints, that Layer 4 responds according to the specified validation rules when evidence is missing, uncertain, or incompatible, and that the final graph exposes validation status, dependencies, evidence, and provenance. This evaluation supports the academic claim that the thesis contributes a traceable reasoning layer for validating and enriching ordered procedural artifacts, rather than a perception system or a complete XR guidance application.
